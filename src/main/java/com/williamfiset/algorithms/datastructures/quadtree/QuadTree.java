...
public class QuadTree {
  ...
  class Node {
    ...
    private void knn(int k, long x, long y, PriorityQueue<SortedPt> heap) {
      ...
      if (radius >= dx) BranchCoverageAnalyser.markCovered("QuadTree.knn.id_182");
      else BranchCoverageAnalyser.markCovered("QuadTree.knn.id_183");
      boolean checkVerticalCell = radius >= dy;
      if (radius >= dy) BranchCoverageAnalyser.markCovered("QuadTree.knn.id_184");
      else BranchCoverageAnalyser.markCovered("QuadTree.knn.id_185");
      ...
      if (heap.size() == k) {
          ...
          if (pointQuadrant == NORTH_WEST) {
            ...
            if (checkHorizontalCell) {
              ...
              if (ne != null) {
                ...
                ne.knn(k, x, y, heap);
              } else BranchCoverageAnalyser.markCovered("QuadTree.knn.id_35");
            } else {
        ...
        for (int quadrant = 1; quadrant <= 4; quadrant++) {
          ...
          if (radius >= dx) BranchCoverageAnalyser.markCovered("QuadTree.knn.id_186");
          else BranchCoverageAnalyser.markCovered("QuadTree.knn.id_187");
          checkVerticalCell = radius >= dy;
          if (radius >= dy) BranchCoverageAnalyser.markCovered("QuadTree.knn.id_188");
          else BranchCoverageAnalyser.markCovered("QuadTree.knn.id_189");
